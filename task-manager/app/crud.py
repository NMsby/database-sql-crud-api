...
def create_user(db: Session, user: schemas.UserCreate):
    ...
    db_user = models.User(
        ...
        email=user.email,
        password_hash=user.password,  # This should be hashed in a real app
...
def get_category_tasks(db: Session, category_id: int, skip: int = 0, limit: int = 100):
    return db.query(models.Task).filter(models.Task.category_id == category_id).offset(skip).limit(limit).all()
